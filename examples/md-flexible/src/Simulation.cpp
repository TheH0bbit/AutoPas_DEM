...
#include "autopas/dem/DEMFunctor.h"
...
extern template bool autopas::AutoPas<ParticleType>::iteratePairwise(autopas::DEMFunctor<ParticleType> *);
  ...
  for (; needsMoreIterations(); ++_iteration) {
    ...
    if (_config->deltaT.value != 0) {
      ...
      if(!dem) {
        ...
        TimeDiscretization::calculatePositionsDEM(autopas, *_particlePropertiesLibrary, _config->deltaT.value);
      }
    ...
    switch (this->_config->functorOption.value) {
      ...
      */
      case MDFlexConfig::FunctorOption::DEM: {
        this->calculateForces<autopas::DEMFunctor<ParticleType>>(autopas);
        break;
      }
    }
    ...
    if (_config->deltaT.value != 0) {
      ...
      if(!dem) {
        TimeDiscretization::calculateVelocitiesDEM(autopas, *_particlePropertiesLibrary, _config->deltaT.value);
      }
      else {
        TimeDiscretization::calculateVelocitiesDEM(autopas, *_particlePropertiesLibrary, _config->deltaT.value);
      }
...
void Simulation::printStatistics(autopas::AutoPas<ParticleType> &autopas) {
  ...
  switch (_config->functorOption.value) {
    ...
    case MDFlexConfig::FunctorOption ::DEM: {
      flopsPerKernelCall = autopas::DEMFunctor<ParticleType>::getNumFlopsPerKernelCall();
      break;
    }
    default:
